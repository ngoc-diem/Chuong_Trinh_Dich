...
void compileAssignSt(void) {
  ...
  int count2 = 0;
  typeRightList[count2++] = compileExpression(); 
  while(lookAhead->tokenType == SB_COMMA){
    eat(SB_COMMA);
    typeRightList[count2++] = compileExpression();
  }
  ...
  if (count1 < count2){
    error(ERR_MISSING_LEFT_VALUE, currentToken->lineNo, currentToken->colNo);
  }
  else if (count2 < count1){
    error(ERR_MISSING_RIGHT_VALUE, currentToken->lineNo, currentToken->colNo);
  }
  else{
    for (int i = 0; i < count1; i++)
    {
      checkTypeEquality(typeLeftList[i], typeRightList[i]);
    }
