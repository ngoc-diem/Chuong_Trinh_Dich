...
Token *readNumber(void)
{
    ...
    while ((currentChar != EOF) && (charCodes[currentChar] == CHAR_DIGIT || charCodes[currentChar] == CHAR_PERIOD))
    {
        ...
        if (charCodes[currentChar] == CHAR_PERIOD)
        {
            if (checkItIsFloat)
            {
                ...
                error(ERR_INVALIDNUMBER, lineCheck, colCheck);
            }
            checkItIsFloat = 1;
        }
    ...
    token->string[count] = '\0';
    token->value = atoi(token->string);
    if (checkItIsFloat == 1 && checkGetRPAR == 0)
    {
        token->tokenType = TK_FLOAT;
    }
    else
...
Token* getToken(void) {
  ...
  if (checkGetRPAR == 1)
    {
        checkGetRPAR = 0;
        readChar();
        return makeToken(SB_RSEL, rparRow, rparCol);
    }
  switch (charCodes[currentChar]) {
  ...
  case CHAR_PERIOD: // gặp dấu ‘.’ 
    ...
    } else if ((currentChar != EOF) &&(charCodes[currentChar] == CHAR_DIGIT))//test float
    {
      // token->tokenType = TK_FLOAT;
